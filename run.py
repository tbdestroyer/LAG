...
if __name__ == '__main__':
    ...
    if args.env == "plane":
        ...
        for j in range(len(data)):
            for i in range(len(data[j]["states"])):
                data[j]["states"][i] = np.array(data[j]["states"][i]).reshape(-1)
                data[j]["next_states"][i] = np.array(data[j]["next_states"][i]).reshape(-1)
                data[j]["actions"][i] = np.array(data[j]["actions"][i]).reshape(-1)
                data[j]["entropy"][i] = np.array(data[j]["entropy"][i]).reshape(())  # scalar
                data[j]["rewards"][i] = np.array(data[j]["rewards"][i]).reshape(())  # scalar
                data[j]["dones"][i] = np.array(data[j]["dones"][i]).reshape(())      # scalar
